...
export default function SettingsPage() {
    ...
    const [editingPendingClientSchedId, setEditingPendingClientSchedId] = useState<string | null>(null);
    ...
    const handleCreateSchedule = async (listId: string, listName: string) => {
        ...
        try {
            ...
            if (editingPendingClientSchedId) {
                await updateDoc(doc(db, "whatsapp_list_schedules", editingPendingClientSchedId), {
                    ...payloadBase,
                });
            } else {
            ...
            // Reset form
            setEditingPendingClientSchedId(null);
            setSchedFormMessage("");
            setSchedFormEventId("");
            setSchedFormDays([]);
            setSchedFormOnceAt("");
                                        ...
                                        <div className="pt-4 space-y-4">
                                            ...
                                            <div className="space-y-3">
                                                {sendingLists.map((list) => {
                                                    ...
                                                    const pendingSchedules = thisListSchedules.filter(s => s.status === "pending_client");
                                                    const activeSchedules = thisListSchedules.filter(s => s.status !== "pending_client");
                                                    ...
                                                    return (
                                                        <div key={list.id} className="border border-gray-200 rounded-xl overflow-hidden">
                                                            ...
                                                            {isScheduling && (
                                                                <div className="border-t border-gray-100 px-4 py-4 space-y-4">
                                                                    <div className="text-xs font-semibold text-gray-600 flex items-center justify-between gap-1.5 flex-wrap">
                                                                        ...
                                                                        <button
                                                                            type="button"
                                                                            onClick={() => {
                                                                                navigator.clipboard.writeText(`${window.location.origin}/client-schedule/${list.id}`);
                                                                                setMessage({ text: "קישור לטופס לקוח הועתק ללוח!", type: "success" });
                                                                            }}
                                                                            className="text-xs px-2.5 py-1.5 rounded bg-orange-50 text-orange-700 hover:bg-orange-100 transition border border-orange-200 flex items-center gap-1.5 font-medium shadow-sm"
                                                                        >
                                                                            <Copy size={12} />
                                                                            קישור לטופס לקוחות
                                                                        </button>
                                                                    ...
                                                                    {/* Pending client schedules */}
                                                                    {pendingSchedules.length > 0 && (
                                                                        <div className="space-y-2 border border-orange-200 bg-orange-50/50 p-3 rounded-xl mt-2 mb-2">
                                                                            <div className="text-xs font-bold text-orange-700 flex items-center gap-1.5 mb-2">
                                                                                <Clock size={12} /> ממתינים לאישור לקוח ({pendingSchedules.length})
                                                                            </div>
                                                                            {pendingSchedules.map(sched => (
                                                                                <div key={sched.id} className="rounded-lg border text-xs overflow-hidden bg-white border-orange-200 shadow-sm relative pt-1">
                                                                                    <div className="flex items-center gap-2 px-3 py-2.5">
                                                                                        <div className="flex-1 min-w-0">
                                                                                            <div className="font-medium text-gray-800 flex items-center gap-1.5 flex-wrap">
                                                                                                הצעת לקוח לתזמון חד-פעמי
                                                                                                {sched.mediaUrl && <span className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded-full flex items-center gap-0.5"><UploadCloud size={9} />מדיה מצורפת</span>}
                                                                                            </div>
                                                                                            <div className="text-gray-500 mt-1 italic pr-2 border-r-2 border-orange-300 line-clamp-2">
                                                                                                "{sched.messageText}"
                                                                                            </div>
                                                                                            <div className="text-gray-400 mt-1.5">
                                                                                                {sched.scheduledAt && (
                                                                                                    <span className="text-orange-600 font-medium whitespace-nowrap">
                                                                                                        מועד מבוקש: {new Date(sched.scheduledAt).toLocaleString("he-IL", { dateStyle: "short", timeStyle: "short", timeZone: "Asia/Jerusalem" })}
                                                                                                    </span>
                                                                                                )}
                                                                                            </div>
                                                                                        </div>
                                                                                        <div className="flex items-center gap-1.5 flex-shrink-0">
                                                                                            <button
                                                                                                type="button"
                                                                                                onClick={() => {
                                                                                                    setEditingPendingClientSchedId(sched.id);
                                                                                                    setSchedFormType("once");
                                                                                                    setSchedFormOnceAt(sched.scheduledAt ? new Date(sched.scheduledAt).toLocaleString('sv-SE', { timeZone: 'Asia/Jerusalem' }).replace(' ', 'T').slice(0, 16) : "");
                                                                                                    setSchedFormSendMode("custom");
                                                                                                    setSchedFormMessage(sched.messageText || "");
                                                                                                    setSchedFormPresetMediaUrl(sched.mediaUrl || "");
                                                                                                    setSchedFormMediaFile(null);
                                                                                                }}
                                                                                                className={`px-3 py-1.5 rounded-lg font-medium transition shadow-sm ${editingPendingClientSchedId === sched.id ? "bg-orange-100 text-orange-800 border border-orange-300" : "bg-orange-500 hover:bg-orange-600 text-white"}`}
                                                                                            >
                                                                                                {editingPendingClientSchedId === sched.id ? "עורך..." : "אשר וערוך"}
                                                                                            </button>
                                                                                            <button
                                                                                                type="button"
                                                                                                onClick={() => handleDeleteSchedule(sched.id)}
                                                                                                className="px-2 py-1.5 rounded-lg border border-red-200 hover:bg-red-50 text-red-500 transition font-medium"
                                                                                            >
                                                                                                דחה
                                                                                            </button>
                                                                                        </div>
                                                                    ...
                                                                    {/* ── New schedule form ── */}
                                                                    <div className={`border border-dashed rounded-xl p-3 space-y-3 transition-colors ${editingPendingClientSchedId ? "bg-orange-50/40 border-orange-300" : "bg-violet-50/40 border-violet-200"}`}>
                                                                        <div className="flex items-center justify-between">
                                                                            <div className={`text-xs font-bold ${editingPendingClientSchedId ? "text-orange-700" : "text-gray-600"}`}>
                                                                                {editingPendingClientSchedId ? "עריכת תזמון מלקוח" : "+ תזמון חדש"}
                                                                            </div>
                                                                            {editingPendingClientSchedId && (
                                                                                <button
                                                                                    type="button"
                                                                                    onClick={() => {
                                                                                        setEditingPendingClientSchedId(null);
                                                                                    ...
                                                                                    className="text-xs text-orange-600 hover:bg-orange-100 px-2 py-0.5 rounded cursor-pointer"
                                                                                >
                                                                                    בטל עריכה
                                                                                </button>
                                                                            )}
